...
from posts.models import Pokemon, Post
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "posts/inicio.html",
                {"mensaje": f"Se ha creado el usuario: {username}"},
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                return render(
                    request, "posts/inicio.html", {"mensaje": f"Bienvenido {usuario}"}
                )
            ...
            return render(
                request, "accounts/login.html", {"mensaje": "Error,forumlario erroneo"}
            )
...
def profile(request, username):
    try:
        ...
        if is_user:
            ...
            posts = Post.objects.filter(autor=user, publicado=True)
